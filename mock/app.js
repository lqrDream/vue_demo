...
server.use((req, res, next) => { //可选 统一修改请求方式
  ...
  if (req.url.includes('login') || req.url.includes('reg')) {
    ...
    if (token && token.length === 15 || token.length === 16) {
      next();
    } else {
      res.jsonp({
        err: 2,
...
server.get(mock + '/login', (req, res) => {
  ...
  (username === 'lqr' && password === 'lqr123') ?
  res.jsonp({ //好比 res.send({})
      "err": 0,
      "msg": "登录成功",
      "data": {
        "nikename": mr.cname(3, 6),
        "sex": mr.boolean(0, 1),
        "iphone": mr.integer(12345678901, 12345678909),
        "date": mr.integer(1511197952100, 1611197952100),
...
server.use(jsonServer.rewriter({
  [mock + "/*"]: "/$1",
  "/banner/check\\?id=:id": "/banner/:id",
  "/banner/add": "/banner",
  "/banner/del?id=:id": "/banner/:id",
  "/home/check\\?id=:id": "/home/:id",
  "/home/add": "/home",
  "/home/del?id=:id": "/home/:id",
  "/follow/check\\?id=:id": "/follow/:id",
  "/follow/add": "/follow",
  "/follow/del?id=:id": "/follow/:id",
  "/column/check\\?id=:id": "/column/:id",
  "/column/add": "/column",
  "/column/del?id=:id": "/column/:id",
  "/user/check\\?id=:id": "/user/:id",
  "/user/add": "/user",
  "/user/del?id=:id": "/user/:id"
}));
